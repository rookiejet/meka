...
//! cannot see or send anything". FreeBSD is the one exception to the first half, and it is the
//! broker's doing rather than a stronger claim here: a jailed command sees the base the operator's
//! prefix policy admits rather than the whole host. Landlock alone also cannot stop a command
//! changing a file's mode, owner, timestamps or extended attributes: no ABI has a right for those,
//! and Bubblewrap's read-only bind is what refuses them.
